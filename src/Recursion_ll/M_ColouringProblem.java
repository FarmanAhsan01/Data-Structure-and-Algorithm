...
public class M_ColouringProblem {
    public static boolean graphColoring(List<Integer>[]G,int []color,int r,int m){
        int n=G.length;
    ...
    public static boolean inSafe(int node,List<Integer>[]G,int[] color,int n,int col){
        for (int it:G[node]){
    ...
    public static boolean solve(int node, List<Integer>[]G,int color[],int n,int m){
        ...
        for (int i=1;i<=m;i++){
            if(inSafe(node+1,G,color,n,m)==true){
                return true;
            }
            color[node]=0;
        }
